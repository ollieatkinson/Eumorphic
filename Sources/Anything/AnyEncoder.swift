//
//  Created by Oliver Atkinson on 07/06/2021.
//

import Foundation

protocol AnyEncoderProtocol: AnyObject, Decoder {
...
open class AnyEncoder: Encoder {
    
    public var codingPath: [CodingKey] = [ ]
    public var userInfo: [CodingUserInfoKey: Any] = [:]
    
    required public init(codingPath: [CodingKey] = [], userInfo: [CodingUserInfoKey: Any] = [:]) {
...
extension AnyEncoder.UnkeyedContainer: UnkeyedEncodingContainer {
    ...
    public mutating func encodeNil() throws {
        defer { count += 1 }
        encoder.codingPath.append(^count)
        defer { encoder.codingPath.removeLast() }
        encoder.value = NSNull()
    }
...
extension AnyEncoder {
    ...
    public func get() -> Any {
        switch _value {
        case let array as [Any]: return array[codingPath] as Any
        case let dictionary as [String: Any]: return dictionary[codingPath] as Any
    ...
    fileprivate func box(_ it: Encodable) throws -> Any {
...
extension AnyEncoder.KeyedContainer {
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> where NestedKey: CodingKey { unsupported() }
...
extension AnyEncoder.UnkeyedContainer {
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> where NestedKey: CodingKey { unsupported() }
    public mutating func nestedUnkeyedContainer() -> UnkeyedEncodingContainer { unsupported() }
    public func superEncoder() -> Encoder { unsupported() }
}

fileprivate protocol OptionalEncodableProtocol: Encodable {
...
extension Optional: OptionalEncodableProtocol where Wrapped: Encodable & Equatable {
    ...
    func encodeUnwrapped(to encoder: AnyEncoder) -> Any {
        switch self {
        case let .some(value): return (try? encoder.encode(value)) as Any
